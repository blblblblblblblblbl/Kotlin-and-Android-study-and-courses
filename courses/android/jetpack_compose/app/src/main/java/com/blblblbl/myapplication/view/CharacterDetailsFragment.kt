...
import com.blblblbl.myapplication.presentation.CharacterDetailsViewModel
...
class CharacterDetailsFragment : Fragment() {
            ...
            setContent {
                info?.let {
                    Column(Modifier.verticalScroll(rememberScrollState())) {
                        GlideImage(imageModel = { info!!.image.toString() }, modifier = Modifier.fillMaxWidth())
                        Column(modifier = Modifier.padding(PaddingValues(30.dp,15.dp))) {
                            ...
                            infoCard(str1 = "species and gender:", str2 = "${info!!.species.toString()}(${info!!.gender.toString()})")
                            infoCard(str1 = "origin:", str2 = "${info!!.origin?.name.toString()}")
                            infoCard(str1 = "location:", str2 = "${info!!.location?.name.toString()}")
                            Text(
                                text = "Episodes:",
                                fontSize = titleFont,
                                fontWeight = FontWeight.Bold
    ...
    /*@Composable fun seriesList(episodesUrls:ArrayList<String>){
            ...
            items(episodes.value){
                for (i in episodes.value.indices){
                    Card(modifier = Modifier
                        ...
                        .fillMaxWidth()
                        .wrapContentHeight()) {
                        Text(text = "${episodes.value[i].name}")
                    }
                }
            }
        }
        ...
        Column( verticalArrangement = Arrangement.spacedBy(4.dp),){
            for (i in episodes.value.indices){
                Card(modifier = Modifier
                    ...
                    Text(text = "${episodes.value[i].episode}", fontSize = smallFont, textAlign = TextAlign.Right )
